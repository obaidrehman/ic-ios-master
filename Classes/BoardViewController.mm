...
@synthesize analysisView, bookMovesView, boardView, whiteClockView, blackClockView, moveListView, gameController, searchStatsView, gameInfo , lblScore;
...
-(void)viewDidLayoutSubviews{
    self.lblTimeWhite.layer.cornerRadius = 5;
    self.lblTimeBlack.layer.cornerRadius = 5;
    self.lblMessageBadge.layer.cornerRadius = 9;
    self.lblMessageBadge.layer.borderWidth = 1;
    self.lblMessageBadge.layer.borderColor = UIColor.whiteColor.CGColor;
    self.lblTimeWhite.clipsToBounds = true;
    self.lblTimeBlack.clipsToBounds = true;
    self.lblMessageBadge.clipsToBounds = true;
}
...
- (void)loadView {
    ...
    //   if (![CommonTasks DeviceTypeiPhone]) // just moved device identifier code to common task, as it may be required several times
    //   {
    //
    //       BOOL portrait = [CommonTasks DeviceOrientationPotrait]; // get device orientation, but note that app will be restricted to potrait in global settings!
    //
    //      // Content view
    //       CGRect appRect = self.appFrame; // assign the pparen view frame size, so that app may scale accordingly
    ...
    //   }
    //   else // we should use this layout for for both iPhone and iPad type devices! // todo?
    //   {
    // iPhone or iPod touch
    
    // Content view
    CGRect appRect = self.appFrame;  // assign the parent view frame size, so that app may scale accordingly
    rootView = [[RootView alloc] initWithFrame: appRect];
    //[rootView setBackgroundColor: [UIColor colorWithRed:0.85 green:0.85 blue:0.85 alpha: 1.0]];
    [rootView setBackgroundColor: [UIColor colorWithRed:0.934 green:0.934 blue:0.953 alpha: 1.0]]; // theme?
    
    
    [[NSBundle mainBundle] loadNibNamed:@"OnlineBoardCustomView" owner:self options:nil];
    ...
    //Move list
    moveListView = [[MoveListView alloc] initWithFrame: CGRectMake(0.0f, 0.0f, appRect.size.width, 20.0f)];
    
    NSLog(@"MOVELISTVIEWFRAMES",moveListView.frame);
...
//    // button options menu
//    self.ButtonOptionsMenu = [[nButton alloc] initWithFrame:CGRectMake(/*appRect.size.width - 30.f - 8.0f*/ 8.0f, 4.0f, 40.0f, 40.0f)];
//    [self.ButtonOptionsMenu SetAsImageOnlyButtonWithImage:[UIImage imageNamed:@"iconOptions"]
//                                        ImageScalingValue:3
//                                           ImageTintColor:[UIColor colorWithHexString:@"#0b2744"]
//                                             CornerRadius:(self.ButtonOptionsMenu.frame.size.width / 2)
...
//    [self.ButtonOptionsMenu addTarget:self action:@selector(ButtonOptionsMenuTapped:) forControlEvents:UIControlEventTouchUpInside];
    
    
//    gameInfo = [[UILabel alloc] initWithFrame:CGRectMake(self.ButtonOptionsMenu.frame.size.width,
//                                                         8.0f,
//                                                         appRect.size.width - self.ButtonOptionsMenu.frame.size.width * 2,
//                                                         30.0f)];
//    [gameInfo setFont:[UIFont systemFontOfSize:14]];
//    [gameInfo setTextAlignment:NSTextAlignmentCenter];
//    [gameInfo setBackgroundColor:[UIColor whiteColor]];
//    [gameInfo setTextColor:[UIColor colorWithHexString:@"#0b2744"]];
//    gameInfo.layer.cornerRadius = 5.0f;
//    gameInfo.clipsToBounds = YES;
//    [gameInfo setText:@"player vs player"];
    
    
//    gameInfo.userInteractionEnabled = YES;
//    UITapGestureRecognizer *tapGesture =
//    [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(GameInfoMenuTapped:)];
//    [gameInfo addGestureRecognizer:tapGesture];
    
//    CGFloat buttonOptionsMenuMargin = self.ButtonOptionsMenu.frame.size.height;
...
//    // Board
//    boardView = [[BoardView alloc] initWithFrame: CGRectMake(0.0f, buttonOptionsMenuMargin + 30.f/*38.0f*//*18.0f*/, appRect.size.width, appRect.size.width)]; // theme?
//    [contentView addSubview: boardView];
//
//
//    // Search stats
//    searchStatsView = [[UILabel alloc] initWithFrame: CGRectMake(0.0f, /*20.0f*/0.0f, /*appRect.size.width*/ 0.0f, /*18.0f*/0.f)];
//    [searchStatsView setFont: [UIFont systemFontOfSize: 14.0]]; // theme?
//    //[searchStatsView setTextAlignment: UITextAlignmentCenter];
...
//    [searchStatsView setBackgroundColor: [UIColor colorWithRed:0.934 green:0.934 blue:0.953 alpha: 1.0]]; // theme?
//    [contentView addSubview: searchStatsView];
    
    
    // Clocks
//    whiteClockView = [[UILabel alloc] initWithFrame: CGRectMake(-30 ,
//                                                                /*20.0f*/ buttonOptionsMenuMargin + 10.0f,
//                                                                0.5f * appRect.size.width - 20.f,
//                                                                18.0f)];
//    [whiteClockView setFont: [UIFont systemFontOfSize: 11.0]]; // theme?
//    [whiteClockView setTextAlignment: NSTextAlignmentCenter]; // theme?
//    [whiteClockView setText: @"White: 5:00"];
//    whiteClockView.textColor = [UIColor whiteColor];
//    //[whiteClockView setBackgroundColor: [UIColor colorWithRed:0.85 green:0.85 blue:0.85 alpha: 1.0]];
//    [whiteClockView setBackgroundColor: [UIColor clearColor]]; //[UIColor colorWithRed:0.934 green:0.934 blue:0.953 alpha: 1.0]]; // theme?
//
//    blackClockView = [[UILabel alloc] initWithFrame: CGRectMake(0.5f * appRect.size.width + 50.0f,
//                                                                /*20.0f*/ buttonOptionsMenuMargin + 10.0f,
//                                                                0.5f * appRect.size.width - 20.f,
//                                                                18.0f)];
//    [blackClockView setFont: [UIFont systemFontOfSize: 11.0]];   // theme?
//    [blackClockView setTextAlignment: NSTextAlignmentCenter];    // theme?
//    [blackClockView setText: @"Black: 5:00"];
//    blackClockView.textColor = [UIColor whiteColor];
//    //[blackClockView setBackgroundColor: [UIColor colorWithRed:0.85 green:0.85 blue:0.85 alpha: 1.0]];
//    [blackClockView setBackgroundColor: [UIColor clearColor]];//[UIColor colorWithRed:0.934 green:0.934 blue:0.953 alpha: 1.0]]; // theme?
//
//    [contentView addSubview: whiteClockView];
//    [contentView addSubview: blackClockView];
...
//    {
//        [contentView addSubview:gameInfo];
//        lblScore = [[UILabel alloc]initWithFrame : CGRectMake(0.5f * appRect.size.width - 70.0f,
//                                                              /*20.0f*/ buttonOptionsMenuMargin + 10.0f,
//                                                              0.5f * appRect.size.width - 20.f,
//                                                              18.0f)];
//        [lblScore setFont: [UIFont systemFontOfSize: 11.0]];   // theme?
//        [lblScore setTextAlignment: NSTextAlignmentCenter];    // theme?
//        [lblScore setText: @"(Score: 0/0)"];
//        lblScore.textColor = [UIColor whiteColor];//[UIColor blackColor];
//        //[blackClockView setBackgroundColor: [UIColor colorWithRed:0.85 green:0.85 blue:0.85 alpha: 1.0]];
//        [lblScore setBackgroundColor: [UIColor clearColor]];//[UIColor colorWithRed:0.934 green:0.934 blue:0.953 alpha: 1.0]]; // theme?
//
//
//
//        [contentView addSubview:lblScore];
//    }
//    [contentView addSubview:self.ButtonOptionsMenu];
    
    
    
    
    // Analysis
//    analysisView = [[UILabel alloc] initWithFrame: CGRectMake(0.0f, appRect.size.width + /*38.0f*/buttonOptionsMenuMargin + 30.f, appRect.size.width, 18.0f)];
//    [analysisView setFont: [UIFont systemFontOfSize: 13.0]];     // theme?
//    //[analysisView setBackgroundColor: [UIColor colorWithRed:0.85 green:0.85 blue:0.85 alpha: 1.0]];
//    [analysisView setBackgroundColor: [UIColor colorWithRed:0.934 green:0.934 blue:0.953 alpha: 1.0]];   // theme?
//    CALayer* layer = [analysisView layer];   // theme?
//    CALayer *bottomBorder = [CALayer layer]; // theme?
//    bottomBorder.borderColor = [UIColor colorWithRed: 0.781 green: 0.777 blue: 0.797 alpha:1.0].CGColor; // theme?
//    bottomBorder.borderWidth = 1;    // theme?
//    bottomBorder.frame = CGRectMake(0, layer.frame.size.height-1, layer.frame.size.width, 0.5);  // theme?
//    [layer addSublayer:bottomBorder];    // theme?
//    [contentView addSubview: analysisView];
...
//    moveListView = [[MoveListView alloc] initWithFrame: CGRectMake(0.0f,
//                                                                   appRect.size.width + /*56.0f*/ buttonOptionsMenuMargin + 50.0f,
//                                                                   appRect.size.width,
//                                                                   frameHeight - appRect.size.width - /*56.0f*/ 36.0f )];//- 44.0f)]; // theme?
//    [contentView addSubview: moveListView];
...
//
    ...
    [activityIndicator setCenter: CGPointMake(0.5f * appRect.size.width, 0.5f * appRect.size.width)];
    [activityIndicator setActivityIndicatorViewStyle: UIActivityIndicatorViewStyleWhite];    // theme?
    [_chessBoardView addSubview: activityIndicator];
    [activityIndicator startAnimating];
      //  }
    ...
    //    if (self.isOfflinePlay)
    //    {
    //        gameMenu = [[UIActionSheet alloc] initWithTitle: nil
    //                                               delegate: self
    ...
    //                                         cancelButtonTitle: @"Cancel"
    //                                    destructiveButtonTitle: nil
    //                                         otherButtonTitles: @"Play white", @"Play black", @"Play both", @"Analysis", nil];
    //
    //        moveMenu = [[UIActionSheet alloc] initWithTitle: nil
    //                                               delegate: self
    //                                      cancelButtonTitle: @"Cancel"
    //                                 destructiveButtonTitle: nil
    //                                      otherButtonTitles: @"Take back", @"Step forward", @"Take back all", @"Step forward all", @"Move list", @"Move now", nil];
    //    }
    //    optionsMenu = nil;
    //    saveMenu = nil;
    //    emailMenu = nil;
...
- (void)ButtonOptionsMenuTapped:(id)sender
{
    [self showButtonOptionsMenuGrid];
}
- (void)GameInfoMenuTapped:(id)sender
{
    [self showUserInfo];
}
...
-(void)showUserInfo
{
    ...
    [message appendFormat:@"%@: %@\n", @"WhiteEngine",self.gameController.game.whiteEngine];
    [message appendFormat:@"%@: %@\n", @"WhiteElo",self.gameController.game.eloWhite];
    [message appendFormat:@"%@: %@\n", @"BlackEngine",self.gameController.game.blackEngine];
    [message appendFormat:@"%@: %@\n", @"BlackElo",self.gameController.game.eloBlack];
    [message appendFormat:@"%@: %@\n", @"Time",self.gameController.game.gameTime];
    
//    RNBlurModalView *alert = [[RNBlurModalView alloc] initWithParentView:self.view title:@"Game Info" message:message];
    //[alert show];
}

- (void)showButtonOptionsMenuGrid
{
    NSArray *items = (self.isOfflinePlay) ?
    @[
//      [[RNGridMenuItem alloc] initWithImage:[UIImage imageNamed:@"iconInprogressGame"] title:@"Game"],
//      [[RNGridMenuItem alloc] initWithImage:[UIImage imageNamed:@"iconSettings"] title:@"Settings"],
//      [[RNGridMenuItem alloc] initWithImage:[UIImage imageNamed:@"iconRefresh"] title:@"Flip"],
//      [[RNGridMenuItem alloc] initWithImage:[UIImage imageNamed:@"iconReturn"] title:@"Quit"],
      ] :
    @[
//      [[RNGridMenuItem alloc] initWithImage:[UIImage imageNamed:@"iconRefresh"] title:@"Flip"],
      ];
...
//{
//    if ([item.title isEqualToString:@"Game"])
//    {
//        [self showGameMenu];
//    }
...
//    }
//    else if ([item.title isEqualToString:@"Flip"])
//    {
//        [gameController rotateBoard];
//    }
//    else if ([item.title isEqualToString:@"Quit"])
//    {
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"StockfishPlayOfflineQuit" object:self userInfo:nil];
//    }
//    else
//    {
//
...
- (void)SetGameInfo:(NSString *)info
{
    ...
    NSArray *arf = [info componentsSeparatedByString:@" "];
    
    
    NSLog(@"infooooooooooo %@ - - - - - %@", arf[0], arf[2]);
    
    
    //self.lblTimeBlack.text =
    self.computerLevelLbl.text = arf[0];
    self.player2.text = arf[2];
...
- (void)showGameMenu
{
    GameViewController *ovc;
    ovc = [[GameViewController alloc] initWithBoardViewControllerAndGameController:self :gameController];
...
- (void)showOptionsMenu {
    ...
    ovc = [[GameOptionsViewController alloc] initWithBoardViewController: self];
    navigationController =
    [[UINavigationController alloc]
     initWithRootViewController: ovc];
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
...
//MARK:- BACK FORWARD action methods
- (IBAction)backMoveBtnAction1:(id)sender {
    [gameController takeBackMove];
    NSLog(@"kdshfkj kjdshjk sdfjk sdjkfhjsdfhkjdsfhkjsdhfjksdhfkjsdhfjkdshfjkshfkjsdhfkjsdhfkjhdskj kjhdskjf sdjkh ");
}

- (IBAction)fwdMoveBtnAction1:(id)sender {
    [gameController replayMove];
    NSLog(@"kdshfkj kjdshjk sdfjk hhhhhh893479875897843578934758943759873489574389578934759843754398579843 kjhdskjf sdjkh ");
...
- (IBAction)topBtnAction:(id)sender {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"dismissOnlineBoardVC" object:self userInfo:nil];
}

- (IBAction)actionButtomBtn:(id)sender {
    ...
    UIAlertAction *resign = [UIAlertAction actionWithTitle:@"Resign" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action)
                           {
                               [[NSNotificationCenter defaultCenter]
                                postNotificationName: @"ResignGameNotification"
                                object: self
                                userInfo:@{@"type":@"resign"}];
                           }];
    
    
    UIAlertAction *draw = [UIAlertAction actionWithTitle:@"Draw" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action)
                           {
                               [[NSNotificationCenter defaultCenter]
                                postNotificationName: @"ResignGameNotification"
                                object: self
    ...
    [actionSheet addAction:resign];
    [actionSheet addAction:cancel];
    
    [self presentViewController:actionSheet animated:YES completion:nil];
}
- (IBAction)actionChatButton:(id)sender {
    [[NSNotificationCenter defaultCenter] postNotificationName: @"chatNotification"
                                                        object: self
                                                      userInfo:nil];
