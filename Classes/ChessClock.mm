   ...
   if (self = [super init]) {
      ...
      timer = [NSTimer scheduledTimerWithTimeInterval: 1.0
       ...
       dispatch_async(dispatch_get_main_queue(), ^{
           ...
           CLS_LOG(@"setting text in chessclock lbl = %@", [NSString stringWithFormat: @"White: %@", [self whiteRemainingTimeString]]);
           [whiteClockView setText: [NSString stringWithFormat: @"White: %@", [self whiteRemainingTimeString]]];
           [blackClockView setTextColor: [UIColor blackColor]];
           [blackClockView setText: [NSString stringWithFormat: @"Black: %@", [self blackRemainingTimeString]]];
       });
   }
   return self;
...
- (void)resetWithWhiteTime:(int)time increment:(int)inc {
    whiteInitialTime = time;
    whiteIncrement = inc;
    whiteConsumedTime = 0;
...
- (void)resetWithBlackTime:(int)time increment:(int)inc {
    blackInitialTime = time;
    blackIncrement = inc;
    blackConsumedTime = 0;
...
- (void)timerWasFired:(NSTimer *)t {
   if (isRunningForWhite) {
    ...
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:@(isRunningForBlack) forKey:@"isRunningForBlack"];
    [[NSNotificationCenter defaultCenter] postNotificationName: @"isRunningForBlack" object:nil userInfo:userInfo];
}
