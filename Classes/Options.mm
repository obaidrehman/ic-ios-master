...
- (id)init {
   if (self = [super init]) {
      ...
      if (![defaults objectForKey: KEY_SHOW_ANALYSIS]) {
         showAnalysis = YES;
         [defaults setBool: YES forKey: KEY_SHOW_ANALYSIS];
      }
      else
      ...
      if (![defaults objectForKey:KEY_SHOW_ARROW]) {
         showArrow = YES;
         [defaults setBool: YES forKey:KEY_SHOW_ARROW];
      } else {
      ...
      if (![defaults objectForKey: KEY_SHOW_BOOK_MOVES]) {
         showBookMoves = YES;
         [defaults setBool: YES forKey: KEY_SHOW_BOOK_MOVES];
      }
      else
      ...
      if (!pieceSet) {
         pieceSet = @"Merida";
         [defaults setObject: @"Merida" forKey: KEY_PIECE_SET];
      }
      ...
      if (![defaults objectForKey: KEY_MOVE_SOUND]) {
         moveSound = YES;
         [defaults setBool: YES forKey: KEY_MOVE_SOUND];
      }
      else
      ...
      if (!colorScheme) {
         colorScheme = @"Blue";
         [defaults setObject: @"Blue" forKey: KEY_COLOR_SCHEME];
      }
...
- (UIColor *)brightHighlightColor {
   return [highlightColor interpolateTowardsColor:[UIColor whiteColor] atPoint:0.67f];
}
...
- (void)setShowAnalysis:(BOOL)shouldShowAnalysis {
   ...
   [[NSUserDefaults standardUserDefaults] setBool: showAnalysis forKey: KEY_SHOW_ANALYSIS];
   [[NSUserDefaults standardUserDefaults] synchronize];
}
...
- (void)setShowArrow:(BOOL)shouldShowArrow {
   ...
   [[NSUserDefaults standardUserDefaults] setBool:showArrow forKey:KEY_SHOW_ARROW];
   [[NSUserDefaults standardUserDefaults] synchronize];
}
