...
@implementation GameController{
    BOOL isMoveUpdateFromServer;
}

@synthesize whiteClockView, blackClockView, searchStatsView, game, rotated, MoveScore;
@dynamic gameMode;
...
- (void)doMoveFrom:(Square)fSq to:(Square)tSq promotion:(PieceType)prom {
   ...
   assert(square_is_ok(tSq));
    NSLog(@"isWhiteORNOT%i",[[MyManager sharedManager] isPlayerWhite]);
   fSq = [self rotateSquare: fSq];
   tSq = [self rotateSquare: tSq];
   ...
   pendingFrom = pendingTo = SQ_NONE;
    ...
    NSString *fen = [game currentFEN];
    NSString *PGN = [game pgnString];
    NSDictionary *dict = @{@"to":[NSNumber numberWithInt:tSq],@"from":[NSNumber numberWithInt:fSq],@"fen":fen,@"PGN":PGN};
    NSLog(@"%@",dict);
    [[NSNotificationCenter defaultCenter] postNotificationName:@"getCurrentMove" object:self userInfo:dict];
...
/// nabeeg for kibitzing always use this
- (void)animateMoveFrom:(Square)fSq to:(Square)tSq promotionPieceType:(PieceType)promotionPiece {
    assert(square_is_ok(fSq));
    assert(square_is_ok(tSq));
    ...
    if (self.gameMode != GAME_MODE_KIBITZ)
        if ([game pieceCanMoveFrom: fSq to: tSq] > 1)
        {
            pendingFrom = fSq;
            pendingTo = tSq;
    ...
    if (castle) {
        ...
        m = make_ep_move(fSq, tSq);
    else if (promotionPiece != NO_PIECE_TYPE)   // added by nabeeg
        m = make_promotion_move(fSq, tSq, promotionPiece);
    else
        m = make_move(fSq, tSq);
...
-(void)playMySound{
    //    AudioServicesPlaySystemSound(pieceSounds[int(2) & 7][random() % 2]);
    SystemSoundID sound1;
    NSURL *soundURL = [[NSBundle mainBundle] URLForResource:@"Bishop1" withExtension:@"wav"];
...
- (void)gameFromPGNString:(NSString *)pgnString
   ...
   [game setTimeControlWithTime: [[Options sharedOptions] baseTime]
                      increment: [[Options sharedOptions] timeIncrement]];
    //fhjjghfjfghj
   // [[game clock] resetWithWhiteTime:600000 increment:1];
    //ghjhgj
   pieceViews = [[NSMutableArray alloc] init];
   pendingFrom = SQ_NONE;
   pendingTo = SQ_NONE;
...
- (void)SetScoreForLastMove:(NSString*) score
{
    [self.MoveScore setText:score];
}

- (void)ShowArrowFromSquare:(Square)from To:(Square)to
{
    if (boardView)
        [boardView showArrowFrom:from to:to];
}
