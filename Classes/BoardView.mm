...
// set karna hai ye masla

//******************************8*******************************************************************************

//MARK:- Highlight square
...
////response sahi karna hai
//MARK:- Move highlighted
...
//             NSString *fen = [[gameController game] currentFEN];
//             NSDictionary *dict = @{@"to":[NSNumber numberWithInt:s],@"from":[NSNumber numberWithInt:fromSquare],@"fen":fen};
//             NSLog(@"%@",dict);
   ...
   if (fromSquare != SQ_NONE) {
      ...
      [self stopHighlighting];
       if ([gameController pieceCanMoveFrom: fSq to: tSq]){
          ...
          NSString *fen = [[gameController game] currentFEN];
           NSString *PGN = [[gameController game] pgnString];
